...
trim_OptimumSteele <- function(trim_window, jags.out, params, data, cal_dates, thin){
...
pred.v2 <- data.frame(model_id = "OptimumSteele",
                      datetime = df$Date,
                      variable = "chlorophyll-a",
                      prediction = unname(pred)
                      )

OptimumSteele_plot <- ggplot()+
  xlab("")+
  ylab("Chla (ug/L)")+
