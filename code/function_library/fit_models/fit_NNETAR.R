...
fit_NNETARs <- function(data, cal_dates){
  ...
  # get fitted values
  fitted_values <- fitted(my.nnetars)
  ...
  #build output df
  #build output df
  df.out <- data.frame(model_id = rep(c("NNETARnoDrivers","NNETAR"), each = length(dates$datetime)),
                       datetime = rep(dates$datetime,2),
                       variable = "chlorophyll-a",
                       prediction = fitted_values$.fitted)
  
  #return output + model with best fit + plot
  return(list(out = df.out, NNETAR = my.nnetars, plot = NNETAR_plot))
}
