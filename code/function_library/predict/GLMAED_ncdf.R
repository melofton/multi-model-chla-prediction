...
GLMAED <- function(spinup_folder = "./code/model_files/GLM-AED/spinup",
  ...
  for(t in 1:length(pred_dates)){
    ...
    if(t == 1 & start_from_spinup == TRUE){
      ...
      message("the_depths")
      all_elevs <- ncdf4::ncvar_get(current_nc, var = "H")
      current_elevs <- all_elevs[which(!is.na(all_elevs[,2])),2] # this is column for noon on day 2 of sim
      the_elevs <- current_elevs[!is.na(current_elevs)]
    ...
    for(i in 1:length(wq_vars)){
      ...
      current_var <- rev(all_current_var[which(!is.na(all_current_var[,ic_col])),ic_col])
      message(length(current_var))
