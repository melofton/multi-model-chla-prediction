...
OptimumMonodNP <- function(data, pred_dates, forecast_horizon, fit){
  ...
  I_K <- fit$summary$statistics[3,1]
  ...
  for(t in 1:length(pred_dates)){
    ...
    pred[i] = pred[i-1] + (pred[i-1] * R_growth * (((wtemp[i-1] - 0) / (Topt - 0)) *((100 - wtemp[i-1]) / (100 - Topt)) ^((100 - Topt) / (Topt - 0))) * ((swr[i-1]/I_K) / (1 + (swr[i-1]/I_K))) * (DIN[i-1]/(DIN[i-1]+ksdin)) * (SRP[i-1]/(SRP[i-1]+kssrp))) - (pred[i-1] * R_resp * (1.08^(wtemp[i-1] - 20))) 
    }
    
    #set up dataframe for today's prediction
    temp.df <- data.frame(model_id = "OptimumMonodNP",
