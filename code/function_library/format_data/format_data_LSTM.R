...
format_data_LSTM <- function(filepath_exo = "./multi-model-ensemble/data/data_raw/FCR_Catwalk_EDI_2018_2022.csv",
...
##SECCHI----
  ...
  df.out <- left_join(met2, inf2) %>%
    ...
    mutate(DOY = yday(Date),
           Flag_DOY = 1) %>%
    select(Date, DOY, AirTemp_C, Shortwave_Wm2, Windspeed_ms, Inflow_cms, WaterTemp_C,
           SRP_ugL, DIN_ugL, LightAttenuation_Kd, Chla_ugL,
           Flag_DOY, Flag_AirTemp_C, Flag_Shortwave_Wm2, Flag_Windspeed_ms, Flag_Inflow_cms, Flag_WaterTemp_C, Flag_SRP_ugL,
  ...
  metadata <- data.frame(column_number = c(1:length(colnames(df.out))),
    ...
    mutate(column_type = ifelse(column_names == "Date","date",
                                ifelse(column_names == "Chla_ugL","target",
