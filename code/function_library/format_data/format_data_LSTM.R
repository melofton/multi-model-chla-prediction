#Format data for LSTM model for chl-a
#Author: Mary Lofton
...
format_data_LSTM <- function(filepath_chemistry = "./data/data_raw/chemistry_2013_2023.csv",
  ...
  df.out <- left_join(met2, inf2) %>%
    ...
    mutate(DOY = yday(datetime),
           Flag_DOY = 1) %>%
    select(datetime, DOY, AirTemp_C_mean, PAR_umolm2s_mean, WindSpeed_ms_mean, Flow_cms_mean, Temp_C_mean,
           SRP_ugL, DIN_ugL, LightAttenuation_Kd, Chla_ugL_mean,
           Flag_DOY, Flag_AirTemp_C_mean, Flag_PAR_umolm2s_mean, Flag_WindSpeed_ms_mean, Flag_Flow_cms_mean, Flag_Temp_C_mean, Flag_SRP_ugL,
           Flag_DIN_ugL, Flag_LightAttenuation_Kd, Flag_Chla_ugL_mean)
  
  metadata <- data.frame(column_number = c(1:length(colnames(df.out))),
                         column_names = colnames(df.out)) %>%
    mutate(column_type = ifelse(column_names == "datetime","date",
                                ifelse(column_names == "Chla_ugL_mean","target",
                                       ifelse(grepl("Flag",column_names),"flag","feature"))))
  
  return(list(df.out = df.out, metadata = metadata))
}
